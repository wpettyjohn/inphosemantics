function main() {
    $.getJSON("../../data/inphosemantics-directory.json", 
		  ...
		  // needs additional parameter req
		  function printTermColumn(limit) {
		      $("#root > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody").each(function() {
			  for (var i=0; i<limit; i++) {
			      var index = i+1;
			      var simObj = termArray[i];
			      var simObjKey = Object.keys(simObj)[0];
			      $(this).append("<tr><td>"+simObjKey+"</td><td>s"+simObj['simObjKey']+"</td></tr>");
			  }
		      });
		  }
		  ...
		  $("#querySubmit").click(function(){
		      ...
		      hashObj = convertHashToObject(hash);
		      renderTable(hashObj['corpora'], hashObj['models'], hashObj['phrases']);
		      var requestData = JSON.stringify(hashObj);
		      xml_http_post("index.html", requestData, printTermColumn(10));
